...
| DT-062 | **Directorio de colaboradores (talent pool)** — registrar UNA vez a cada persona con la que se trabaja (**nombre · función/rol · contacto · ciudad**) para reusarla en contratos futuros. El **Equipo Operativo** de cada proyecto = **SELECCIÓN del directorio** (many-to-many: una persona en varios proyectos). Se conecta con el **Rol** de las tareas + los **portales por colaborador** (Paso 2 D-105). **Decisión de modelo:** directorio **COMPARTIDO entre gremios** (un camarógrafo sirve a G\*S y G\*Startups) — excepción deliberada al aislamiento por `línea` (las personas son recurso compartido, no data de gremio); la **asignación** a un proyecto sí carga la línea. | G\*Startups / Equipo Operativo (2026-07-30) | **Media** — fold al build del Equipo Operativo. Pendiente confirmar con Ian: ¿compartido (recomendado) o por línea? |
