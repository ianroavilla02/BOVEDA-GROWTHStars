...
| DT-064 | **Fase 0 en el modelo de fases (Cumbre / G\*Startups)** — el plan real tiene **4 fases** (Fase 0 Preproducción 2-17 ago + Pre-lanzamiento 15D + Evento 3D + Sostenimiento 10D), pero el dashboard tiene 3 y la que llama "Fase I — Preproducción" es en realidad el **Pre-lanzamiento**. La **Fase 0** (build de marca/plantillas/CRM — trabajo real pagado con el 1er 50%) **no existe**. Agregar Fase 0 + relabelear Fase I → Pre-lanzamiento. | Análisis plan Cumbre (2026-07-31) | **ALTA** — prerequisito para poblar el calendario (tiene ~15 días de tareas Fase 0 sin dónde vivir) |
| DT-065 | **Split de honorarios por colaborador (talent pool)** — el presupuesto de la Cumbre divide los $8M por persona (Juan $3.05M AV / Ian $4.95M estrategia+diseño+CRM), desglosado por servicio/fase. El dashboard trackea el revenue del cliente pero NO el split interno (cuánto cobra cada colaborador). Conecta con DT-062 (talent pool): cada colaborador con su share por proyecto. Para operar como negocio (pagar al equipo). | Análisis plan Cumbre (2026-07-31) | **Media** — no bloquea el arranque |
| DT-066 | **Lock dates + checkpoints con alerta de umbral (G\*Startups)** — la ruta crítica tiene fechas inamovibles (Logo Lock) y checkpoints condicionales (Ola 1 < 70% → activar refuerzo). Encaja con "alertar-no-actuar" (D-105) pero no está como feature. Resolver como tareas-hito con alerta de umbral. NOTA: el Test Ledger (10 experimentos GX) NO se construye — queda en el doc de estrategia hasta que se necesite tracking vivo. | Análisis plan Cumbre (2026-07-31) | **Media** |
| DT-062 | **Directorio de colaboradores (talent pool)** — registrar UNA vez a cada persona con la que se trabaja (**nombre · función/rol · contacto · ciudad**) para reusarla en contratos futuros. El **Equipo Operativo** de cada proyecto = **SELECCIÓN del directorio** (many-to-many: una persona en varios proyectos). Se conecta con el **Rol** de las tareas + los **portales por colaborador** (Paso 2 D-105). **Decisión de modelo (Ian, 2026-07-30):** directorio **SEPARADO por gremio** (scoped por `línea` — cada workspace ve sus colaboradores). PERO un colaborador puede pertenecer a **uno o AMBOS gremios** (many-to-many persona↔gremio, **sin duplicar el registro**) — ej. Juan Mora y Andrés Fierro (directores AV de G\*S) que harán trabajos en G\*Startups → aparecen en ambos directorios. La **asignación** a un proyecto carga la línea. Los roles son seleccionables/agregables por contrato (como ya está). | G\*Startups / Equipo Operativo (2026-07-30) | **Media** — fold al build del Equipo Operativo |
| DT-063 | **Entregable obligatorio por tarea → auto-"Terminado" (G\*Startups)** — cada tarea tiene un **entregable** (link O documento). Regla de status: **SIN entregable** la tarea solo puede ser "Sin iniciar" o "En progreso"; al **adjuntar el entregable se marca automáticamente "Terminado"** (evidencia = entregado). **"Aprobado"** = validación manual de Ian (paso final tras Terminado, es el único responsable). Reusar el mecanismo de adjuntos/Storage existente. Nota: el auto-Terminado NO contradice "alertar-no-actuar" (D-105) — es un estado **DERIVADO de un hecho** (existe el entregable), no una acción silenciosa que esconde un problema. | G\*Startups / tareas (2026-07-30) | **Media** — parte del build de tareas de la Cumbre |
| DT-061 | **Pipeline de desarrollo web in-home (G*Startups)** — estructura de agentes para dejar de subcontratar (~$1200 USD/página) y capturar el margen. **(1) DECIDIR EL STACK primero** (2 carriles: **Shopify/WooCommerce** para tiendas que opera el cliente; **UN stack custom estándar** —ej. Astro/Next+Tailwind+CMS headless— para landings/custom; NUNCA snowflakes distintos = la estandarización es el multiplicador). **(2) Pipeline tipo Jefe de Área** (molde Auditor Algorítmico): Spec → Frontend → Backend/Integraciones (pagos, pixels, forms) → 🔒 **QA+Seguridad (GATE obligatorio** para sitios con pago) → Deploy+Handoff → Sostenimiento. **(3) Verdad del sostenimiento:** los agentes ASISTEN, no eliminan; Ian sigue dueño del "¿está vivo?". Reductores reales = stack aburrido + estandarización + cliente edita su propio contenido (CMS). **(4) Cobrar con tier de soporte**, no mantenimiento gratis eterno. Pasarelas Colombia: Wompi/Mercado Pago/PayU/ePayco (nunca tocar datos de tarjeta = PCI). | Consulta CTO (2026-07-27) | **ALTA — días** (entran 2 contratos de desarrollo web). **Pendiente de Ian:** qué tipo de web predomina (tienda/landing/corporativa) → decide el stack principal, y sobre eso se diseña el pipeline exacto |
