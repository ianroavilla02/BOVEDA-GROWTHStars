...
| DT-059 | **Toasts que mienten:** el dashboard tiene 71 mutaciones (POST/PUT/DELETE/PATCH) en `index.html` y solo 26 chequeos de `.ok` → hay handlers que muestran toast de ÉXITO aunque el server responda 404/500. Confirmado en "+ AÑADIR MISIÓN" (el POST no existía y `showToast('Mision creada')` se disparaba igual con 404 — por eso el sistema tuvo 0 misiones y nadie lo notó **desde siempre**) | Paridad Vista de Sello (2026-07-16) | **Alta** — bloquea D-102: el switch de estado de pago es una mutación nueva; si nace con este patrón, dirá "Pagada" sin escribir el `.md` |
