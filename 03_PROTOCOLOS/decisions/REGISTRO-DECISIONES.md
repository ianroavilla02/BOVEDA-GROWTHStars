...
- **Separación VISUAL por workspace (2026-07-27):** el dashboard central tiene un **switcher arriba: `G*Stars ⟷ G*Startups`.** Todo (sidebar de clientes, métricas, proyectos) filtra por el workspace activo → dos ambientes en paralelo sin saturar la lista. **NO es separación de datos** (mismo Supabase/bóveda) — es navegación. Las sub-áreas de cada línea (MGMT/AV/Bookings/Eventos en G\*S; Growth Hacking/sueltos en G\*Startups) viven **dentro** de su workspace.
