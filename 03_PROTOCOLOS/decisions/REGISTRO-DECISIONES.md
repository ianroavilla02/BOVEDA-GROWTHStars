...
- **Aislamiento por línea (2026-07-27, refinado 2026-07-30):** el dashboard central tiene un **switcher arriba: `G*Stars ⟷ G*Startups`.** Cada gremio es su **propio mundo AISLADO**: métricas, agentes, tipos de contrato, equipo y DATA (clientes/proyectos/cobros/tareas) **scopeados por una dimensión `línea`** → los dos gremios (Artístico G\*S / Corporativo G\*Startups) **NUNCA se cruzan.** **Es aislamiento LÓGICO** (misma base Supabase, todo tagueado por `línea`), **NO dos bases físicas** — la separación física solo se justificaría con un equipo independiente por gremio o un spin-off, y no es el caso (Ian opera ambos). **Misma estructura reusada, data separada.** Los tipos de contrato son similares pero propios por línea (G\*S: MGMT/AV/Eventos · G\*Startups: Growth Hacking/Web/Promocional/Activación). Las sub-áreas (Bookings, Eventos, etc.) viven dentro de su workspace.
